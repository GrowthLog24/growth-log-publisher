...
운영진 컴퓨터의 로그인된 전용 Chrome을 이용해 방송대 게시 작업과 티스토리 임시저장을 돕는 Electron 앱입니다. Electron은 로컬 자동화 서버와 트레이 앱을 실행하고, 이미 연결된 외부 운영 웹의 자동화 요청을 처리합니다.
...
실행 방식은 `npm start` 하나로 통일합니다. 개발 중 브라우저 로그인 상태는 저장소의 `.knou-playwright-profile/`에 저장되며, 이 디렉터리는 Git과 배포 패키지에서 제외됩니다. 패키지로 설치한 앱의 로그인 상태는 사용자별 앱 데이터에 저장됩니다.
...
`npm start`가 Electron 트레이 앱과 로컬 자동화 서버를 함께 실행합니다. 로컬 작업 웹은 제공하지 않으며, 연결된 외부 운영 웹에서 작업을 실행합니다.
